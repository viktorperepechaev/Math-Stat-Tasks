...
def generate_theta_values() -> np.ndarray:
    return np.arange(
        10 * left_boundary_theta,
        10 * right_boundary_theta + 1,
        1,
        dtype=float,
    ) / 10
...
def exact_confidence_interval(sample: np.ndarray) -> tuple[float, float]:
    ...
    quantile: float = float(norm.ppf(1.0 - alpha / 2.0))

    left: float = sample_mean - quantile / np.sqrt(n)
    right: float = sample_mean + quantile / np.sqrt(n)
...
def graphic_intervals(
    ...
    canvas.write_html(f"T11_n_{n}.html")
